...
import { PaginationDto } from '@modules/books/dto';
...
describe('BookService', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        BookService,
        { provide: getModelToken('Books'), useFactory: mockBookModel },
      ],
    }).compile();
    bookService = module.get<BookService>(BookService);
    bookModel = module.get<Model<BookDocument>>(getModelToken('Books'));
  });
  ...
  describe('findOne', () => {
    it('Should return an user', async () => {
      bookModel.findOne.mockImplementationOnce(() => ({
        lean: jest.fn().mockResolvedValue(oneBook),
      }));
  ...
  describe('findManyByGenre', () => {
    it('Should return array books without pagination', async () => {
      ...
      bookModel.find.mockImplementationOnce(() => ({
        lean: jest.fn().mockResolvedValue(manyBooks),
      }));
    ...
    it('Should return array books with pagination', async () => {
      bookModel.countDocuments.mockResolvedValue(1);
      bookModel.find.mockImplementationOnce(() => ({
        skip: jest.fn().mockImplementationOnce(() => ({
          limit: jest.fn().mockImplementationOnce(() => ({
            lean: jest.fn().mockResolvedValueOnce(manyBooksWithPagination),
          })),
        })),
      }));
      const pagination: PaginationDto = { limit: 2, page: 1 };
      const result = await bookService.findManyByGenre(
        'some-genre',
        pagination,
      );
